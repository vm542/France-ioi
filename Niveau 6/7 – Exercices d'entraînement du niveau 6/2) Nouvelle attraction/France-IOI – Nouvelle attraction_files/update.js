...
op.test=test||op.test||location.hash==="#test-bu";op.ignorecookie=op.ignorecookie||location.hash==="#ignorecookie-bu";op.reasons=[];op.hide_reasons=[];function check_show(op){var bb=op.browser=$bu_.get_browser(op.override_ua);op.is_below_required=required[bb.n]&&$bu_.less(bb.fullv,required[bb.n])===1;if(bb.other!==false)
op.hide_reasons.push("is other browser:"+bb.other)
if(bb.esr&&!op.notify_esr)
op.hide_reasons.push("Extended support (ESR)")
if(bb.mobile&&op.mobile===false)
op.hide_reasons.push("do not notify mobile")
if(op.apiver<2018.5||op.apiver>2020.1){if(bb.is_latest)
...
return;op.setCookie=function(hours){document.cookie='browserupdateorg=pause; expires='+(new Date(new Date().getTime()+3600000*hours)).toGMTString()+'; path=/; SameSite=None; Secure';};if(op.already_shown&&(op.ignorecookie||op.test))
op.setCookie(-10)
if(op.reminder>0)
